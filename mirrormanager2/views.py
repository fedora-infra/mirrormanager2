...
from mirrormanager2.lib import constants, model
...
def propagation_all():
    return flask.redirect(flask.url_for("base.propagation", prefix="development"))


@views.route("/propagation/<prefix>")
def propagation(prefix):
    ...
    return flask.render_template(
        "propagation.html", repos=repos, prefix=prefix, labels=labels, datasets=datasets
    )
