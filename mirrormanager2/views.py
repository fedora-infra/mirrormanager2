...
from mirrormanager2.perms import (
    login_required, admin_required, is_site_admin, is_mirrormanager_admin,
)
...
def mysite():
    ...
    sites = mmlib.get_user_sites(
        flask.g.db, flask.g.fas_user.username)
    return flask.render_template(
        'my_sites.html',
        tag='mysites',
    ...
    if form.validate_on_submit():
        ...
        try:
            msg = mmlib.add_admin_to_site(
                flask.g.db, site, flask.g.fas_user.username)
            flask.flash(msg)
        except SQLAlchemyError as err:  # pragma: no cover
...
@views.route(
    '/host/<host_id>/host_acl_ip/<host_acl_ip_id>/delete',
    methods=['POST']
)
@login_required
...
@views.route(
    '/host/<host_id>/host_netblock/<host_netblock_id>/delete',
    methods=['POST']
)
@login_required
...
@views.route(
    '/host/<host_id>/host_asn/<host_asn_id>/delete',
    methods=['POST']
)
@admin_required
...
def host_category_url_new(host_id, hc_id):
    ...
    form.url.validators[2].regex = re.compile(
        flask.current_app.config.get('MM_PROTOCOL_REGEX', ''),
        re.IGNORECASE
    )
