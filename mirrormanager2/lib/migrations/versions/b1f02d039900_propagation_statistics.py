...
def upgrade():
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["version_id"], ["version.id"], name=op.f("fk_propagation_stat_version_id_version")
        ),
        sa.PrimaryKeyConstraint("version_id", "datetime", name=op.f("pk_propagation_stat")),
    )
    op.create_index(
        op.f("ix_propagation_stat_date"), "propagation_stat", ["datetime"], unique=False
    )
    op.create_index(
        op.f("ix_propagation_stat_version_id"), "propagation_stat", ["version_id"], unique=False
    )


def downgrade():
    op.drop_index(op.f("ix_propagation_stat_version_id"), table_name="propagation_stat")
