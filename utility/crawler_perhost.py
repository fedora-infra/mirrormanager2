...
def select_host_categories_to_scan(session, options):
    result = []
    if len(options.categories):
        for c in options.categories:
            hc = mirrormanager2.lib.get_host_category_by_hostid_category(
                session, host_id=options.hostid, category=category)
            for entry in hc:
                result.append(hc)

    else:
        host = mirrormanager2.lib.get_host(session, options.hostid)
...
def per_host(session, host, options):
    ...
    for hc in host_categories_to_scan:
        ...
        for d in trydirs:
            ...
            try:
                ...
                mark_not_up2date(
                    sys.exc_info(), host, "Unknown exception raised.  "
...
def main():
    ...
    parser.add_option(
        "-c", "--config", dest="config", default='dev.cfg',
        help="TurboGears config file to use")

    parser.add_option(
        "--hostid", dest="hostid", type='int', default=None,
    ...
    parser.add_option(
        "--timeout-minutes", type="int", dest="timeout_minutes", default=0,
        help="Minutes to let the crawler run before killed "
    ...
    if options.logfile is not None:
        ...
        # fixme - http and FTP debugging is still getting printed to
        # stdout from the httplib module
    if options.debug:
    ...
    try:
        rc = per_host(session, host.id, options)
    except TimeoutException:
#        mark_not_up2date(
#            None, host.id,
